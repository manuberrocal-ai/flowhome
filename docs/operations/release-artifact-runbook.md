...
Runner-local paths are initialized in the first step of each job and exported through `GITHUB_ENV` for subsequent steps. Do not move `runner.temp` expressions into job-level `env`: that context is unavailable there and GitHub rejects the workflow before any job runs. The verification manifest and the downloaded production manifest intentionally have different locations. This initialization does not bypass the manual production condition, environment approval or artifact checks. See [GitHub context availability](https://docs.github.com/en/actions/reference/workflows-and-actions/contexts#context-availability).
