...
The repository runs in Basic Consent Mode: no optional provider script or measurement event is loaded before an explicit `accepted` choice. The build must also select `PUBLIC_APP_ENV=production`, `PUBLIC_ANALYTICS_ENABLED=true`, a valid, reviewed `PUBLIC_GTM_ID`, and the matching `PUBLIC_GA4_ID`. Local and staging builds reject enabled analytics. These are build-time settings, not runtime switches: changing a Pages variable cannot enable or disable an already-built artifact. See the [environment runbook](operations/environment-runbook.md). `PUBLIC_CLARITY_ID` is optional. `PUBLIC_GA4_ID` identifies the Google destination to stop synchronously on revocation; it does not load another tag or prove account access. Review all destinations in GTM before enabling the build.
